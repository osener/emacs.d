...
Trace vars using clojure.tools.trace (from [[https://github.com/clojure-emacs/nrepl.el/pull/329][here]])
#+BEGIN_SRC emacs-lisp
  ...
  (define-key nrepl-interaction-mode-map (kbd "C-c M-t") 'nrepl-toggle-trace)
#+END_SRC

Use persistent REPL history
#+BEGIN_SRC emacs-lisp
  (setq nrepl-history-file (expand-file-name ".nrepl-history"
...
Display search matches on mode-line
#+BEGIN_SRC emacs-lisp
...
Only trigger completion if the last command has been an editing command
#+BEGIN_SRC emacs-lisp
